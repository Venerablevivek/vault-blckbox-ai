...
describe('integrity, quotas and workspace deletion', () => {
  ...
  const usage = async () =>
    (await h.query<{ storage_used_bytes: string }>('SELECT storage_used_bytes FROM workspaces WHERE id = $1', [alice.workspaceId]))[0]!
      .storage_used_bytes;
  ...
  describe('storage quota', () => {
    ...
    it('refuses a file that does not fit, storing nothing', async () => {
      await h.query('UPDATE workspaces SET storage_quota_bytes = $2 WHERE id = $1', [alice.workspaceId, SAMPLE_PDF.length + 10]);
    ...
    it('never lets concurrent uploads overshoot the quota, and leaves no orphaned objects', async () => {
      await h.query('UPDATE workspaces SET storage_quota_bytes = $2 WHERE id = $1', [alice.workspaceId, Math.floor(SAMPLE_PDF.length * 1.5)]);
      const results = await Promise.all(
        [1, 2, 3].map((i) => uploadDocument(h.app, alice.cookie, alice.workspaceId, `race-${i}.pdf`, pdf(`race ${i}`.padEnd(24, '.')))),
      );
  ...
  describe('deleting a workspace', () => {
    ...
    beforeEach(async () => {
      ...
      storageKey = (await h.query<{ storage_key: string }>('SELECT storage_key FROM documents WHERE id = $1', [documentId]))[0]!.storage_key;
    ...
    it('is owner-only and requires the exact name', async () => {
      const member = await call('DELETE', `/api/workspaces/${alice.workspaceId}`, bob.cookie, { confirmName: 'Acme Legal' });
      expect(member.statusCode).toBe(403);

      const wrong = await call('DELETE', `/api/workspaces/${alice.workspaceId}`, alice.cookie, { confirmName: 'acme legal' });
      expect(wrong.statusCode).toBe(400);
      expect(wrong.json().error.code).toBe('CONFIRMATION_MISMATCH');

      const outsider = await registerUser(h.app, 'eve@example.com');
      expect((await call('DELETE', `/api/workspaces/${alice.workspaceId}`, outsider.cookie, { confirmName: 'Acme Legal' })).statusCode).toBe(404);
    });

    it('ends access for everyone at once and kills its links and invitations', async () => {
      const pending = await call('POST', `/api/workspaces/${alice.workspaceId}/invitations`, alice.cookie, { email: 'carol@example.com' });
      const inviteToken = pending.json().inviteUrl.split('/invite/')[1];

      expect((await call('DELETE', `/api/workspaces/${alice.workspaceId}`, alice.cookie, { confirmName: 'Acme Legal' })).statusCode).toBe(204);
    ...
    it('tells the other members, without naming a workspace they can no longer open', async () => {
      ...
      await settle();
      const notes = (await call('GET', '/api/notifications', bob.cookie)).json().notifications as Array<{ type: string; title: string }>;
      expect(notes.some((n) => n.type === 'workspace.deleted' && n.title === 'Acme Legal was deleted')).toBe(true);
      const aliceNotes = (await call('GET', '/api/notifications', alice.cookie)).json().notifications as Array<{ type: string }>;
    ...
    it('cleans up nested folders when the workspace row is removed', async () => {
      const parent = (await call('POST', `/api/workspaces/${alice.workspaceId}/folders`, alice.cookie, { name: 'Parent' })).json().folder.id;
      await call('POST', `/api/workspaces/${alice.workspaceId}/folders`, alice.cookie, { name: 'Child', parentId: parent });
