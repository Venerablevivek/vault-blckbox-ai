...
typing. The blueprint owns the scope, and its §10 exclusion list was given to the agent in the
prompts. *(Planned but not done: an `AGENTS.md` file carrying that list, so it was a standing rule
rather than something repeated per session.)*
...
| Layering rules (kept by review; no lint rule exists) | Test scaffolding from the cases I enumerate |
...
5. **Small commits with real messages**, so the git history shows the direction, not one 4,000-line
   "initial commit".
6. **Rules the agent can't drift from** were meant to live in `CLAUDE.md` plus an ESLint
   `no-restricted-imports` rule. *Neither was created*; the rules lived in the design docs and in
   review, which is weaker and is listed as a follow-up.
...
| --- | --- | --- |
| Authorization checks inlined into handlers, subtly different each time | Each route is generated in isolation | One `policy.ts` plus an explicit `requireMember()` per handler (not a prefix guard); the table-driven cross-tenant test fails if a route skips it |
| Object key built from the user-supplied filename | It's the "natural" thing to write | `storage/keys.ts` is the only constructor; a test asserts keys are UUID-only (no lint rule) |
...
| Object deleted before the DB row commits | Reads top-to-bottom naturally | Spec'd ordering + `storage-failure.test.ts`, which deletes a document and checks the row and the object (there is no GC job) |
| `SELECT * FROM documents WHERE id = $1` without the tenant filter | The id feels unique enough | Documents loaded by id go through `authorizeById()`, which checks membership of the row's workspace before returning anything; the cross-tenant suite covers every route |
| Presigned URL signed against the internal MinIO host | The internal client is already in scope | It fails immediately in the browser with `SignatureDoesNotMatch`; the dual-client split is spec'd in `technical.md` §7 |
| `deleted_at` filter forgotten in one query | Easy to miss on the tenth repository method | Written into each query by hand (there is no shared helper); a test soft-deletes and asserts absence from listing, download and share resolution |
...
| Over-engineering: a generic RBAC engine, an event bus, a repository framework | Agents reach for extensible abstractions unprompted | "No abstraction without a second caller", applied in review (no `CLAUDE.md` was created) |
...
| **`workspace_id` missing from a `WHERE` clause** | The document id feels unique enough on its own | Listing queries carry `workspace_id`; id-addressed documents go through `authorizeById()`; the cross-tenant suite covers the two id-only routes explicitly |
| **String-interpolated SQL** | It reads more naturally than `$1, $2` | Review rejects any `${}` near SQL (no lint rule); the one interpolation that remains is a compile-time constant |
...
| **Scope creep — a VIEWER role, folders, versioning, a component library** | Agents reach for completeness and for extensible abstractions unprompted | The blueprint's §10 list, restated in prompts (no `AGENTS.md` was created) |
