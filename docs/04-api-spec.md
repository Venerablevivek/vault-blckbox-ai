...
105 operations. Auth: *public* (no credential), *session or API token*,
...
| Method | Route | What it does | Auth |
| --- | --- | --- | --- |
| POST | `/api/workspaces/:workspaceId/file-requests` | Create a file request (an upload link for people outside the workspace) | session or API token |
| GET | `/api/workspaces/:workspaceId/file-requests` | A workspace's file requests, newest first | session or API token |
| GET | `/api/file-requests/:id/files` | Files received through a request, with who sent them | session or API token |
| DELETE | `/api/file-requests/:id` | Close a file request | session or API token |
| GET | `/api/requests/:token` | What a file request asks for (public) | public |
| POST | `/api/requests/:token/files` | Send a file through a file request (public, multipart) | public |
